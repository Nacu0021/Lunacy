        ...
        public static void Player_GrabUpdateIL(ILContext il)
        {
                ...
                ))
            {
                ...
                c.EmitDelegate<Func<int, Player, bool, int>>((original, player, eu) =>
                {
                    for (int i = 0; i < 2; i++)
                    {
                        if (player.grasps[i] != null && player.grasps[i].grabbed is CoralSpear s)
                        {
                            int noti = i == 0 ? 1 : 0;
                ...
                x => x.MatchStfld<Weapon>("rotationSpeed")))
            {
                   ...
                   x => x.MatchLdfld<PlayerGraphics>("hands")))
                {
                    ...
                    c.EmitDelegate<Func<Player, int, bool, int, int>>((player, i, eu, orig) =>
                    {
                        ...
                        {
                            if (player.input[0].jmp) player.ReleaseGrasp(i); // Make player do a slight boost up later
                            ...
                            if (player.input[0].pckp)
                            {
                                ...
                                s.increment = true;
                                s.setRotation = Vector3.Slerp(s.coral.rot, s.coral.rot + Custom.DirVec(s.firstChunk.pos, player.mainBodyChunk.pos) * 0.1f, fac).normalized;
